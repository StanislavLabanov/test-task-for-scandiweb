...
class ImagesProductInBusket extends React.Component {
   ...
   leftTransform = () => {
      this.state.offset > -200
         ? this.setState({ offset: -200 * (this.props.images.length - 1) })
         : this.setState({ offset: this.state.offset + 200 })
   }

   rightTransform = () => {
      - this.state.offset > 200 * (this.props.images.length - 2)
   ...
   render() {
      return (
         <div className="images_conteiner">
            <div className="images_flex_block" style={{ width: 200 * this.props.images.length, left: this.state.offset + 'px' }}>
               {
